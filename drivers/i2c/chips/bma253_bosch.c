...
static int bma253_set_range(struct i2c_client *client, unsigned char Range)
{
	...
	if (0 <= Range && Range < 4) {
		...
		E("%s: Range(=%d) is not correct, it should be in 0~3\n", __func__,Range);
		comres = -1;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0 ; i < 0x40; i++) {
		...
		count += snprintf(&buf[count], PAGE_SIZE - count, "0x%x: %d\n", i, reg[i]);
	}
	return count;
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "chip_layout = %d\n"
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "gs_kvalue = 0x%x\n", bma253->pdata->gs_kvalue);
}
...
static ssize_t get_nvram_cali_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "nvram_gs_kvalue = 0x%08x\n", bma253->nvram_gs_kvalue);
	return ret;
}
...
static ssize_t set_nvram_cali_data_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	sscanf(buf, "%d", &bma253->nvram_gs_kvalue);
...
static DEVICE_ATTR(bandwidth, 0664,
...
static DEVICE_ATTR(cali_value, 0440,
...
static DEVICE_ATTR(fast_calibration_x, 0664,
		bma253_fast_calibration_x_show,
		bma253_fast_calibration_x_store);
static DEVICE_ATTR(fast_calibration_y, 0664,
		bma253_fast_calibration_y_show,
		bma253_fast_calibration_y_store);
static DEVICE_ATTR(fast_calibration_z, 0664,
...
static DEVICE_ATTR(eeprom_writing, 0664,
		NULL, bma253_eeprom_writing_store);

static DEVICE_ATTR(chip_layout, 0664,
		bma253_chip_layout_show, bma253_chip_layout_store);

static DEVICE_ATTR(get_raw_data, 0440,
		bma253_get_raw_data_show, NULL);

static DEVICE_ATTR(set_k_value, 0664,
		bma253_set_k_value_show, bma253_set_k_value_store);

#ifdef IS_MTK_PLATFORM
static DEVICE_ATTR(nvram_cali_data, 0664,
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "bma253->user_calibration_flag = %d, bma253->user_offset_buf[0] = %d, bma253->user_offset_buf[1] = %d, bma253->user_offset_buf[2] = %d\n",
