...
//#define HX_LOADIN_CONFIG 1
#define SUPPORT_FINGER_DATA_CHECKSUM 0x0F
#define TS_WAKE_LOCK_TIMEOUT		(2 * HZ)
#define FW_DTGS_VER  0x50
...
static unsigned char i_CTPM_FW_C0X[]=
{
	////#include "Booyi_T50KB110S3M_NewBund_C06_2016-02-22_1811.i"
	//#include "Booyi_T50KB110S3M_NewBund_C10_2016-04-13_2001.i"
};
static unsigned char i_CTPM_FW_C5X[]=
{
	////#include "Booyi_T50KB110S3M_NewBund_C52_2016-04-13_2002.i"
	//#include "Booyi_T50KB110S3M_NewBund_C53_2016-04-18_2139.i"
};
#endif
...
#endif
unsigned char  g_hq_ctp_module_fw_version;  //extern
...
static int himax_chip_self_test_M(uint8_t valuebuf[16]);
...
//	{.compatible = "mediatek,cap_touch_himax852" },	
...
set_bit(KEY_double_click, ts->input_dev->keybit);
...
if(private_ts->vendor_config_ver<FW_DTGS_VER){  /////////<C50///////////////////////////////
	I("deng, himax FW version < C50\n");
	ImageBuffer = i_CTPM_FW_C0X;
	fullFileLength = sizeof(i_CTPM_FW_C0X);
	
	I("IMAGE FW_VER=%x,%x.\n",i_CTPM_FW_C0X[FW_VER_MAJ_FLASH_ADDR],i_CTPM_FW_C0X[FW_VER_MIN_FLASH_ADDR]);
	I("IMAGE CFG_VER=%x.\n",i_CTPM_FW_C0X[FW_CFG_VER_FLASH_ADDR]);
	if (( private_ts->vendor_fw_ver_H < i_CTPM_FW_C0X[FW_VER_MAJ_FLASH_ADDR] )
		|| ( private_ts->vendor_fw_ver_L < i_CTPM_FW_C0X[FW_VER_MIN_FLASH_ADDR] )
		|| ( private_ts->vendor_config_ver < i_CTPM_FW_C0X[FW_CFG_VER_FLASH_ADDR] )
		|| ( himax_calculateChecksum(false) == 0 ))
		{
			...
			if(fts_ctpm_fw_upgrade_with_fs(ImageBuffer,fullFileLength,true) == 0)
				...
				{
					I("%s: TP upgrade OK\n", __func__);
					private_ts->vendor_fw_ver_H = i_CTPM_FW_C0X[FW_VER_MAJ_FLASH_ADDR];
					private_ts->vendor_fw_ver_L = i_CTPM_FW_C0X[FW_VER_MIN_FLASH_ADDR];
					private_ts->vendor_config_ver = i_CTPM_FW_C0X[FW_CFG_VER_FLASH_ADDR];
				}
#ifdef HX_RST_PIN_FUNC
	...
	I("deng, himax FW version > C50\n");
	ImageBuffer = i_CTPM_FW_C5X;
	fullFileLength = sizeof(i_CTPM_FW_C5X);
	
	I("IMAGE FW_VER=%x,%x.\n",i_CTPM_FW_C5X[FW_VER_MAJ_FLASH_ADDR],i_CTPM_FW_C5X[FW_VER_MIN_FLASH_ADDR]);
	I("IMAGE CFG_VER=%x.\n",i_CTPM_FW_C5X[FW_CFG_VER_FLASH_ADDR]);
	if (( private_ts->vendor_fw_ver_H < i_CTPM_FW_C5X[FW_VER_MAJ_FLASH_ADDR] )
		|| ( private_ts->vendor_fw_ver_L < i_CTPM_FW_C5X[FW_VER_MIN_FLASH_ADDR] )
		|| ( private_ts->vendor_config_ver < i_CTPM_FW_C5X[FW_CFG_VER_FLASH_ADDR] )
		|| ( himax_calculateChecksum(false) == 0 ))
		{
#ifdef HX_RST_PIN_FUNC
			himax_HW_reset(false,true);
			...
			if(fts_ctpm_fw_upgrade_with_fs(ImageBuffer,fullFileLength,true) == 0)
				...
				{
					I("%s: TP upgrade OK\n", __func__);
					private_ts->vendor_fw_ver_H = i_CTPM_FW_C5X[FW_VER_MAJ_FLASH_ADDR];
					private_ts->vendor_fw_ver_L = i_CTPM_FW_C5X[FW_VER_MIN_FLASH_ADDR];
					private_ts->vendor_config_ver = i_CTPM_FW_C5X[FW_CFG_VER_FLASH_ADDR];
				}
#ifdef HX_RST_PIN_FUNC
...
//		I("%s, config_selected, %X\n", __func__ ,(uint32_t)config_selected);
		I("%s, config_selected, %lu\n", __func__ ,(unsigned long)config_selected);
		if(config_load == false)
			{
...
#ifdef HX_TP_SHOW_FW_STATUS  /*-------SHOW_FW_STATUS------------*/
u8 hx_fw_version;   //save FW version

static ssize_t tp_fw_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "C%02x\n", hx_fw_version);
} 
...
static int read_FW_version(struct i2c_client *client)
{
	...
	for(i=0;i<3;i++){
		ret = i2c_himax_read(client, HX_VER_FW_CFG, data, 1, 3);
		if(ret){
			E("%s: read fail,and read again: %d! \n", __func__,i);
			fwversion = 255;  // set 0xff  
		}
		else{
			E("deng, read_FW_version(): succeed!\n");
			fwversion = data[0];
			break;
		}
	}
...
static void himax_read_TP_info(struct i2c_client *client)
{
	...
	g_hq_ctp_module_fw_version = private_ts->vendor_config_ver;
	//read sensor ID
...
#ifdef HX_TP_SYS_SELF_TEST   /*--start---S_SELF_TEST------*/
...
static int himax_chip_self_test_M(uint8_t valuebuf[16])
{
	...
	i2c_himax_write(private_ts->client, 0xB1, &valuebuf[0],8,DEFAULT_RETRY_CNT);
	msleep(50);
...
static ssize_t himax_chip_self_test_function(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	val = himax_chip_self_test_M(valuebuf);
	...
	msleep(100);
		...
		for(k=0;k<8;k++) {
		ret+=sprintf(buf + ret ," 0x%x",valuebuf[k]);
		}
	himax_int_enable(private_ts->client->irq,1,false);
...
static int himax_sys_self_test_init(void)
{
	int ret;
	printk("himax_sys_self_test_init\n");
	android_touch_kobj = kobject_create_and_add("android_touch", NULL); 
	if (android_touch_kobj == NULL) {
			ret = -ENOMEM;
			E("register sys_self_test failed. ret = %d\n", ret);
			return ret;
	}

	ret = sysfs_create_file(android_touch_kobj, &dev_attr_tp_self_test.attr);
	if (ret) {
			E("create sys_self_test failed. ret = %d\n", ret);
			return ret;
	}
	 

	I("himax_sys_self_test_init success\n");
	return ret;
	
}

static void himax_sys_self_test_deinit(void)
{
...
static ssize_t himax_gesture_debug_show(struct kobject *kobj,struct kobj_attribute *attr, char *buf)
{
	...
	for(j = 0;j<=15;j++)
	{
		printk("frank_zhonghua:gesture_point_readbuf[%d] = %c\n",j,gesture_point_readbuf[j]);
...
{	
	...
	struct himax_ts_data *ts;
	struct himax_i2c_platform_data *pdata;
	printk("frank_zhonghua:Himax_probe_start\n");
        printk("frank_zhonghua1:%s,%x\n",__func__,client->addr);
...
wake_lock_init(&ts->ts_flash_wake_lock, WAKE_LOCK_SUSPEND, HIMAX852xes_NAME);
    ...
    #endif /*HX_SMART_WAKEUP*/
	#ifdef HX_TP_SYS_SELF_TEST
	himax_sys_self_test_init();
	#endif
#endif
...
#ifdef HX_TP_SHOW_FW_STATUS  /*-------SHOW_FW_STATUS------------*/
	hx_fw_version = read_FW_version(client);
...
if(HX_ON_HAND_SHAKING)//chip on hand shaking,wait hand shaking
{
	...
	if (ts->pdata->powerOff3V3 && ts->pdata->power)
		ts->pdata->power(0);
#ifdef HX_SUSPEND_CLOSE_VGP
	if (regulator_disable(tpd->reg))        /*close 2.8v */
		E("Failed to disable reg-vgp 2.8\n");
...
#ifdef HX_SUSPEND_CLOSE_VGP
	if (regulator_enable(tpd->reg))     /*open 2.8v */
		E("Failed to enable reg-vgp 2.8\n");
...
//#ifdef CONFIG_OF_TOUCH
//	I("Himax 852xES CONFIG_OF_TOUCH is set\n");
//#else
//	I("Himax 852xES CONFIG_OF_TOUCH is NOT set\n");
//#endif
