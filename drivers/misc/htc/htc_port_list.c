...
//check the port list count, return 0 if TCP list over 128, if not, return 1
int check_TCP_port_list_count()
{
	int ret = 0;
	uint count = 0;
	struct list_head *listptr;
	...
	if ( count <= PORT_LIST_COUNT_MAX ) {
		ret = 1;//return 1 if count not over 128
	}
...
static void update_port_list(void)
{
	...
	PF_LOG_INFO("%s: [Port list] Total TCP count=[%d]\n", __func__, count);
...
static int allocate_port_list(void)
{
	...
	port_list = kmalloc(sizeof(uint16_t)*256, GFP_KERNEL);
	#else
	port_list = kmalloc(sizeof(uint16_t)*128, GFP_KERNEL);
	#endif
...
static int __init port_list_init(void)
{
	...
	if (get_kernel_flag() & KERNEL_FLAG_RIL_DBG_MEMCPY)//Mars
		ril_debug_flag = 1;
