...
#define PROC_MAIN_CAMERA_INFO "driver/main_camera_info"
#define PROC_SUB_CAMERA_INFO "driver/sub_camera_info"
#define yl_camera_info_size 64
static char mtk_main_name[yl_camera_info_size] = {0};
static char mtk_sub_name[yl_camera_info_size] = {0};
static char camera_module_name[yl_camera_info_size] = { 0 };
#endif
...
#ifdef CONFIG_YL_CAM_MODULE_NAME
int set_camera_module_name(char* buf)
{
	int len = 0;

	if (buf == NULL) {
		PK_ERR("error buf is NULL\n");
		return -1;
	}
	...
	if (len < 0 || len > 64) {
		PK_ERR("get_device_info error with len %d\n", len);
		return -1;
	}

	memset(camera_module_name, 0, yl_camera_info_size);
	strcpy(camera_module_name, buf);

	return 0;
}
EXPORT_SYMBOL(set_camera_module_name);
#endif
...
//HTC	kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM *)g_invokeSocketIdx, g_invokeSensorNameStr, true, CAMERA_HW_DRVNAME1);
	...
	if (gDrvIndex == 0x10000) {
		...
		#ifdef CONFIG_YL_CAM_MODULE_NAME
		memset(mtk_main_name, 0, yl_camera_info_size);
		memset(mtk_sub_name, 0, yl_camera_info_size);
		#endif
	}
		...
		printk("[camdebug] g_invokeSensorNameStr[%d] %s mtk_ccm_name %s \n", i, g_invokeSensorNameStr[i], mtk_ccm_name);
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {    /* not implement this feature ID */
					...
					#ifdef CONFIG_YL_CAM_MODULE_NAME
					if(1 == g_invokeSocketIdx[i]) {
						snprintf(mtk_main_name, sizeof(mtk_main_name), "%s", camera_module_name);
					} else {
						snprintf(mtk_sub_name, sizeof(mtk_sub_name), "%s", camera_module_name);
					}
					#endif
					err = ERROR_NONE;
				}
...
#ifdef CONFIG_YL_CAM_MODULE_NAME
static int subsys_main_camera_info_read(struct seq_file *m, void *v)
{
	PK_ERR("subsys_main_camera_info_read %s\n", mtk_main_name);
	seq_printf(m, "%s\n", mtk_main_name);
	return 0;
};

static int proc_main_camera_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, subsys_main_camera_info_read, NULL);
};

static  struct file_operations main_camera_proc_fops = {
	.owner = THIS_MODULE,
	.open  = proc_main_camera_info_open,
	.read  = seq_read,
	.release = single_release,
};

static int subsys_sub_camera_info_read(struct seq_file *m, void *v)
{
	PK_ERR("subsys_sub_camera_info_read %s\n", mtk_sub_name);
	seq_printf(m, "%s\n", mtk_sub_name);
	return 0;
};

static int proc_sub_camera_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, subsys_sub_camera_info_read, NULL);
};

static  struct file_operations sub_camera_proc_fops = {
	.owner = THIS_MODULE,
	.open  = proc_sub_camera_info_open,
	.read  = seq_read,
	.release = single_release,
};
#endif
...
static int __init CAMERA_HW_i2C_init(void)
{
	...
	memset(mtk_main_name, 0, yl_camera_info_size);
	memset(mtk_sub_name, 0, yl_camera_info_size);
	proc_create(PROC_MAIN_CAMERA_INFO, 0, NULL, &main_camera_proc_fops);
	proc_create(PROC_SUB_CAMERA_INFO, 0, NULL, &sub_camera_proc_fops);
	#endif
