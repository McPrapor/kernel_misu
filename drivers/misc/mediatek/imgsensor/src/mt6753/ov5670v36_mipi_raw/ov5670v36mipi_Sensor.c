...
#include "ov5670v36mipi_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		.pclk = 111420000,//102850000,				//record different mode's pclk
		.linelength = 1816,//1676,				//record different mode's linelength
	...
	.cap = {
		.pclk = 111420000,//102850000,
		.linelength = 1816,//1676,
		.framelength = 2045,
		.startx = 1,
		.starty = 0,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x380d, 0x18); // ;8c
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x380c,  ((imgsensor_info.pre.linelength >> 8) & 0xFF)); // hts						  
	write_cmos_sensor(0x380d,(imgsensor_info.pre.linelength & 0xFF)); 	   // hts							  
	write_cmos_sensor(0x380e,  ((imgsensor_info.pre.framelength >> 8) & 0xFF));  // vts 						  
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150) { //15fps for PIP
		...
		write_cmos_sensor(0x380a, 0x07); 
		write_cmos_sensor(0x380b, 0x98); 
	    write_cmos_sensor(0x380c, ((imgsensor_info.cap.linelength >> 8) & 0xFF)); // hts
		write_cmos_sensor(0x380d, (imgsensor_info.cap.linelength & 0xFF)); 	   // hts
		write_cmos_sensor(0x380e, ((imgsensor_info.cap.framelength >> 8) & 0xFF));  // vts 
...
static const char *ov5670Vendor = "OmniVision";
static const char *ov5670NAME = "ov5670_front";
static const char *ov5670Size = "5.0M";

static ssize_t sensor_vendor_show(struct device *dev,
                                 struct device_attribute *attr, char *buf)
{
  ssize_t ret = 0;
  sprintf(buf, "%s %s %s\n", ov5670Vendor, ov5670NAME, ov5670Size);
  ret = strlen(buf) + 1;
...
static struct kobject *android_ov5670 = NULL;

static int ov5670_sysfs_init(void)
{
  int ret ;
  LOG_INF("kobject creat and add\n");
  if(android_ov5670 == NULL){
    android_ov5670 = kobject_create_and_add("android_camera2", NULL);
    if (android_ov5670 == NULL) {
    ...
    ret = sysfs_create_file(android_ov5670, &dev_attr_sensor.attr);
    if (ret) {
      LOG_INF("sysfs_create_file " \
             "failed\n");
      kobject_del(android_ov5670);
      android_ov5670 = NULL;
    }
  }
  return 0 ;
