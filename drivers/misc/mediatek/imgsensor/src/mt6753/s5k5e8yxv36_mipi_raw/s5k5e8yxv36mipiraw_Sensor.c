...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		.pclk = 167900000,				//record different mode's pclk
...
int s5k5e8_read_otp_wb(struct s5k5e8_otp_struct *otp)
{
	...
	s5k5e8_start_read_otp(0xe);
	awb_flag1= read_cmos_sensor(0x0A05);
	awb_flag2= read_cmos_sensor(0x0A15);
	...
	if(awb_flag1==0x01)
	{
	    ...
	    otp->u_r_h = read_cmos_sensor(0x0A0d);
	    otp->u_r_l = read_cmos_sensor(0x0A0e);
	    otp->u_b_h = read_cmos_sensor(0x0A0f);
	    otp->u_b_l = read_cmos_sensor(0x0A10);
	    otp->u_g_h = read_cmos_sensor(0x0A11);   
	    otp->u_g_l = read_cmos_sensor(0x0A12);
	  ...
	  else if(awb_flag2== 0X01)
	{
		

		
	    otp->u_r_h = read_cmos_sensor(0x0A1d);
	    otp->u_r_l = read_cmos_sensor(0x0A1e);
	    otp->u_b_h = read_cmos_sensor(0x0A1f);
	    otp->u_b_l = read_cmos_sensor(0x0A20);
	    otp->u_g_h = read_cmos_sensor(0x0A21);
	    otp->u_g_l = read_cmos_sensor(0x0A22);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x0136,0x1A);	//160126	26Mhz
	...
	write_cmos_sensor(0x0307,0x9B); //pll_multiplier	//160126 A8->9B
	write_cmos_sensor(0x0308,0x30); //pll2_multiplier	//160126 34->30
	write_cmos_sensor(0x0309,0x02); //op_pix_clk_div	//160126 42->02
	...
	//requested_link_bit_rate_mbps  836Mbps/lane  ox0344	//160126 832Mbps/lane 0x0340
	write_cmos_sensor(0x0820,0x03);
	write_cmos_sensor(0x0821,0x40); //160126 832Mbps/lane 0x0340
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150){
		...
		write_cmos_sensor(0x0307,0X9B); //160126	A8->9B
		write_cmos_sensor(0x0308,0X17); //160126	19->17
	...
	//Delay 1 frame 
	  mDELAY(42); 
		write_cmos_sensor(0x0136,0X1A);	//160126	18->1A
		write_cmos_sensor(0x0137,0X00);
		write_cmos_sensor(0x0305,0X06);
		write_cmos_sensor(0x0306,0X18);
		write_cmos_sensor(0x0307,0X7E);  //160126	98->7E
		write_cmos_sensor(0x0308,0X32);  //160126	32->30
		write_cmos_sensor(0x0309,0X02);  //20150713
		write_cmos_sensor(0x3C1F,0X00);
		write_cmos_sensor(0x3C17,0X00);
		write_cmos_sensor(0x3C0B,0X04);
...
static const char *s5k5e8Vendor = "SAMSUNG";
static const char *s5k5e8NAME = "s5k5e8_front";
static const char *s5k5e8Size = "5.0M";

static ssize_t sensor_vendor_show(struct device *dev,
                                 struct device_attribute *attr, char *buf)
{
  ssize_t ret = 0;
  sprintf(buf, "%s %s %s\n", s5k5e8Vendor, s5k5e8NAME, s5k5e8Size);
  ret = strlen(buf) + 1;
...
static struct kobject *android_s5k5e8 = NULL;

static int s5k5e8_sysfs_init(void)
{
  int ret ;
  LOG_INF("kobject creat and add\n");
  if(android_s5k5e8 == NULL){
    android_s5k5e8 = kobject_create_and_add("android_camera2", NULL);
    if (android_s5k5e8 == NULL) {
    ...
    ret = sysfs_create_file(android_s5k5e8, &dev_attr_sensor.attr);
    if (ret) {
      LOG_INF("sysfs_create_file " \
             "failed\n");
      kobject_del(android_s5k5e8);
      android_s5k5e8 = NULL;
    }
  }
  return 0 ;
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == imgsensor_info.sensor_id) {
                s5k5e8_sysfs_init();
                LOG_INF("[ryf]i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
