...
#include "ov13850v36mipiraw_Sensor.h"
#include "ov13850v36_otp.h"

#if 1
extern int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId);
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);
extern int ov13850lenmin, ov13850lenmax;
...
//	int temp=0;
	...
	//set 0x5002[1] to "0"
	int temp1=0;
	LOG_INF("zhangdong otp read 0x7226 = 0x%x \n", OV13850_R2A_read_i2c(0x7226));
	LOG_INF("zhangdong otp read 0x7227 = 0x%x \n", OV13850_R2A_read_i2c(0x7227));
	LOG_INF("zhangdong otp read 0x7228 = 0x%x \n", OV13850_R2A_read_i2c(0x7228));
	LOG_INF("zhangdong otp read 0x7229 = 0x%x \n", OV13850_R2A_read_i2c(0x7229));
	
	LOG_INF("zhangdong otp read 0x7232 = 0x%x \n", OV13850_R2A_read_i2c(0x7232));
	LOG_INF("zhangdong otp read 0x7233 = 0x%x \n", OV13850_R2A_read_i2c(0x7233));
	LOG_INF("zhangdong otp read 0x7234 = 0x%x \n", OV13850_R2A_read_i2c(0x7234));
	LOG_INF("zhangdong otp read 0x7235 = 0x%x \n", OV13850_R2A_read_i2c(0x7235));
	...
	LOG_INF("[zj] WB calibration data : %x \n", otp_flag);
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x7221; // base address of info group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x722d; // base address of info group 2
	}
	if(addr != 0) {
		...
		(*otp_ptr).rg_ratio = (OV13850_R2A_read_i2c(addr + 5)<<8) | OV13850_R2A_read_i2c(addr + 6);
		(*otp_ptr).bg_ratio = (OV13850_R2A_read_i2c(addr + 7)<<8) | OV13850_R2A_read_i2c(addr + 8);
		
		LOG_INF("[zj] module_integrator_id=0x%x\n",(*otp_ptr).module_integrator_id);
		LOG_INF("[zj] lens_id=0x%x\n",(*otp_ptr).lens_id);
		LOG_INF("[zj] production_year=0x%x\n",(*otp_ptr).production_year);
		LOG_INF("[zj] production_month=0x%x\n",(*otp_ptr).production_month);
		LOG_INF("[zj] production_day=0x%x\n",(*otp_ptr).production_day);
		LOG_INF("zhangdong otp read rg_ratio = 0x%4x, bg_ratio = 0x%4x \n", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
	}
	else {
	...
	// OTP VCM Calibration
	otp_flag = OV13850_R2A_read_i2c(0x73b0);
	LOG_INF(" VCM calibration data : %x \n", otp_flag);
	addr = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x73b1; // base address of VCM Calibration group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x73b6; // base address of VCM Calibration group 2
	}
	if(addr != 0) {
		(*otp_ptr).flag |= 0x20;
		//temp = OV13850_R2A_read_i2c(addr + 2);
		(* otp_ptr).VCM_start = (OV13850_R2A_read_i2c(addr)<<8) | OV13850_R2A_read_i2c(addr+1);
		(* otp_ptr).VCM_end = (OV13850_R2A_read_i2c(addr + 2) << 8) | OV13850_R2A_read_i2c(addr+3);
		(* otp_ptr).VCM_dir = 0x02;//(temp>>2) & 0x03;
		ov13850lenmin = otp_ptr->VCM_start;
		ov13850lenmax = otp_ptr->VCM_end;
		LOG_INF("focus otp max %d min %d\n",ov13850lenmin, ov13850lenmax);
	}
	else {
		(* otp_ptr).VCM_start = 0;
		(* otp_ptr).VCM_end = 0;
	...
	otp_flag = OV13850_R2A_read_i2c(0x7239);
	LOG_INF(" Lenc calibration data : %x \n", otp_flag);
	addr = 0;
	//int checksumLSC = 0, checksumOTP = 0, checksumTotal = 0;
	if((otp_flag & 0xc0) == 0x40) {
		addr = 0x723a; // base address of Lenc Calibration group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x72f5; // base address of Lenc Calibration group 2
	}
	//
...
if(addr != 0) {
		...
		(* otp_ptr).checksumOTP=checksumOTP;//OV13850_R2A_read_i2c(addr + 187);
		(* otp_ptr).checksumTotal=checksumTotal;//OV13850_R2A_read_i2c(addr + 188);
