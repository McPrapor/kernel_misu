...
static imgsensor_info_struct imgsensor_info = {
    ...
    .slim_video = {
        .pclk = 240000000,
        .linelength = 2968,//2400,
        .framelength = 842,//3328,
        .startx = 0,
        .starty = 0,
...
{{ 4256, 3152, 0, 4, 4256, 3152, 2128, 1572, 8, 2, 2112, 1568, 0, 0, 2096, 1552}, // Preview
 ...
 { 4256, 3152, 840, 850, 2576, 1448, 1288, 724, 4, 2, 1280, 720, 0, 0, 1280, 720}, //hight speed video
 { 4256, 3152, 576, 704, 3104, 1744, 776, 436, 4, 2, 768, 432, 0, 0, 768, 432}, //slow motion
};
#endif
...
//    kal_uint32 frame_length = 0;
    ...
    if (imgsensor.frame_length > imgsensor_info.max_frame_length)
        imgsensor.frame_length = imgsensor_info.max_frame_length;
      //HTC_START, manual shutter mode //Start calculate Line_length_pck
     ...
     else//if(adjusted_line_length_pclk < original_line_length_pclk)
     {
    ...
    if (imgsensor.autoflicker_en) {
     ...
     //HTC_START, manual shutter mode //Update L_P
...
; SystemCLK   :60Mhz
; FPS	        :60 ( = 60M*4/ (HTS*VTS) )
; HTS		:2400(R380c:R380d)
; VTS		:1666(R380e:R380f)
; Tline 	:10us
;---------------------------------------------

20 0100 00 ; stream off
...
*/
    ...
    //// mipi PLL
    write_cmos_sensor(0x0300,0x01);
    write_cmos_sensor(0x0302,0x28); // 32
    ...
    write_cmos_sensor(0x3802,0x03); // 00
    write_cmos_sensor(0x3803,0x54); // 04
    write_cmos_sensor(0x3804,0x0d);
    write_cmos_sensor(0x3805,0x57);
    write_cmos_sensor(0x3806,0x08); // 0c
    write_cmos_sensor(0x3807,0xfb); // d3 // cf // 4b
    write_cmos_sensor(0x3808,0x05);
    write_cmos_sensor(0x3809,0x00);
    write_cmos_sensor(0x380a,0x02); // 06
    write_cmos_sensor(0x380b,0xd0); // 20
    write_cmos_sensor(0x380c,0x09);
    write_cmos_sensor(0x380d,0x60);
    write_cmos_sensor(0x380e,0x06);
...
; resolution: 768x432
; Mipi : 4 lane
; Mipi data rate: 300Mbps/Lane
; SystemCLK   :60Mhz (60*4)
; FPS	        :96.04
; HTS		:2968(R380c:R380d)
; VTS		:842(R380e:R380f)
; Tline 	:12.37us
;---------------------------------------------
...
int ov13850lenmin = 0, ov13850lenmax = 0;
...
static DEVICE_ATTR(otp, 0444, otp_show, NULL);
...
static int ov13850main_sysfs_init(void)
{
	...
	if(first){
		...
		ret = sysfs_create_file(android_ov13850main, &dev_attr_otp.attr);
		if (ret) {
			LOG_INF("sysfs_create_file otp" \
			"failed\n");
		}
