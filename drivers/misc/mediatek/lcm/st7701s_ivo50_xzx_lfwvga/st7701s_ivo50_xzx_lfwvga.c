...
#if defined(BUILD_LK)
	printf("st7701s_ivo50_xzx_lfwvga adc_vol is %d\n",adc_vol);
#else
	printk("st7701s_ivo50_xzx_lfwvga adc_vol is %d\n",adc_vol);
#endif

	return (adc_vol>40&&adc_vol<120) ? 0 : 1;
}
...
#if defined(BUILD_LK)
	printf("st7701s_ivo50_xzx_lfwvga %s,line=%d, id = 0x%x, buffer[0]=0x%08x,buffer[1]=0x%08x\n",__func__,__LINE__, device_id, buffer[0],buffer[1]);
#else
	printk("st7701s_ivo50_xzx_lfwvga %s,line=%d, id = 0x%x, buffer[0]=0x%08x,buffer[1]=0x%08x\n",__func__,__LINE__, device_id, buffer[0],buffer[1]);
#endif
...
// ---------------------------------------------------------------------------
LCM_DRIVER st7701s_ivo50_xzx_lfwvga_lcm_drv=
{
  .name			  = "st7701s_ivo50_xzx_lfwvga",
