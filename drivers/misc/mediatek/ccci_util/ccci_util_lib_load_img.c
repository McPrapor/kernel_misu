...
static int load_image(int md_id, struct ccci_image_info *img_inf, char post_fix[])
{
	...
	if (img_type == IMG_MD) {
		...
		/*step2:check if need to cipher */
				CCCI_UTIL_INF_MSG_WITH_ID(md_id, "masp_ccci_is_cipherfmt pre check");
#ifdef ENABLE_MD_IMG_SECURITY_FEATURE
				CCCI_UTIL_INF_MSG_WITH_ID(md_id, "masp_ccci_is_cipherfmt check");
                                ret = masp_ccci_is_cipherfmt(fp_id, offset, &img_len);
				CCCI_UTIL_INF_MSG_WITH_ID(md_id, "masp_ccci_is_cipherfmt ret=%d!\n", ret);
