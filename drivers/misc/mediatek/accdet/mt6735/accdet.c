...
// add by baibo@yulong 20150313 hookswitch test in factory mode begin
#ifdef AUDIO_AUTO_TEST
#define COMIP_SWITCH_HOOK_TEST_FILE    "driver/hook_test"

ssize_t comip_switch_hook_test_read_proc(struct file *filp, char __user *buf,
										 size_t count, loff_t *f_pos)
{
    ...
    len = scnprintf(msg, size, "button_status=%d, hook_test=0x%x, call_state=%d.\n",
        button_status, g_hook_test, call_status);

	if (copy_to_user(buf, msg + *f_pos, count)) {
		ACCDET_ERROR("copy_to_user failed\n");
		ret = -EFAULT;
		return ret;
	}
	*f_pos += count;
...
ssize_t comip_switch_hook_test_write_proc(struct file *file, const char __user *buffer,
										  size_t count, loff_t *f_pos)
{
    char kbuf[128];
    char *buf = kbuf;
	...
	*f_pos = 0;
    ...
    printk(KERN_INFO "comip_switch_hook_test_write_proc hook_test=%d.\n", val);

    #if 0
    mutex_lock(&data->lock);
    comip_switch_button_power_set(data);
...
static const struct file_operations hook_test_fops = {
    .write = comip_switch_hook_test_write_proc,
    .read = comip_switch_hook_test_read_proc,
};
#endif
// add by baibo@yulong 20150313 hookswitch test in factory mode end
...
#if 0
    hook_test_entry = create_proc_entry(COMIP_SWITCH_HOOK_TEST_FILE, 0666, NULL);
    if (hook_test_entry != NULL)
    {
        hook_test_entry->data       = NULL;
        hook_test_entry->read_proc  = comip_switch_hook_test_read_proc;
        hook_test_entry->write_proc = comip_switch_hook_test_write_proc;
    }
#endif
...
#if 0
#ifdef AUDIO_AUTO_TEST
    remove_proc_entry(COMIP_SWITCH_HOOK_TEST_FILE, NULL);
#endif
#endif
// add by baibo@yulong 20150313 hookswitch test in factory mode end
